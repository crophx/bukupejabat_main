                ...
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50 text-[11px] uppercase text-slate-500 font-black tracking-wider">
                        <tr>
                            ...
                            <th className="p-4 border-b border-slate-100">Telepon</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan="6" className="p-10 text-center text-slate-400 italic">Belum ada pegawai di unit ini.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            {units.length > 0 && (
                <div className="p-4 border-t border-slate-100 flex justify-between items-center">
                    <span className="text-sm text-slate-500 font-medium">
                        Menampilkan {indexOfFirst + 1} - {Math.min(indexOfLast, units.length)} dari {units.length}
                    </span>
            ...
            <dialog id="modal_edit_pegawai" className="modal modal-bottom sm:modal-middle">
                <div className="modal-box bg-white max-w-2xl rounded-3xl p-8 border border-slate-100 shadow-2xl">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <form method="dialog">
                            <button className="btn btn-sm btn-circle btn-ghost text-slate-400">✕</button>
                        </form>
                    </div>
                    ...
                    <form className="space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div className="form-control">
                                ...
                                <input 
                                    type="text" 
                                    defaultValue={selectedUnit?.nama_pegawai || ""} 
                        ...
                        <div className="modal-action flex gap-3 pt-4">
                            <form method="dialog">
                                <button className="btn btn-ghost text-slate-400 font-bold uppercase text-[10px] tracking-widest">Batal</button>
                            </form>
                            <button type="submit" className="btn bg-sky-600 hover:bg-sky-700 border-none text-white px-10 rounded-2xl shadow-xl shadow-sky-100 transition-all font-bold text-xs uppercase">
                                Simpan Perubahan
                            </button>
                        </div>
                    </form>
